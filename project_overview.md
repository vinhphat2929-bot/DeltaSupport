...
| **Gọi sang** | `process/layout.py`, `process/logic.py`, `process/renderers.py`, `process/service.py`, `process/handlers_ui.py`, `stores/task_store.py` |
...
| **Gợi ý** | Đang được refactor dần sang các module con. Tiếp tục tách logic save/update ra `handlers_actions.py` |
...
| `pages/process_page.py` | ~80KB | Đang refactor — cần tách action handlers tiếp |
